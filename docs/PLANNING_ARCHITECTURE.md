...
Le module `/modules/fleet` expose le référentiel canonique `vessels` sous le nom « BBTM - Flotte » et l’onglet « Décision d’effectif ». Toutes les listes de navires continuent de lire cette table unique, initialement alimentée par la liste SharePoint QHSE `543b9f00-aed2-489a-808a-7b64cc835a83`. Une suppression fonctionnelle archive le navire afin de préserver ses références historiques. Dans le Planning, une alerte administrateur ouvre la composition de la bordée : la fonction RH reste immuable et la fonction Planning est confirmée sur l’affectation continue complète. Seul le libellé RH exact `Capitaine` peut recevoir la fonction Planning `Capitaine`. Un brevet manquant reste visible et bloquant, sauf dérogation explicite, motivée et auditée dans `planning_staffing_derogations`. La RPC de publication refuse toute diffusion tant qu’un blocage subsiste ; un effectif compris entre minimum et cible reste un avertissement.
