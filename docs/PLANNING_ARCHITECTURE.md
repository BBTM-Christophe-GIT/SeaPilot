...
Les contrôles distinguent Information, Avertissement et Blocage pour les doubles affectations, absences, indisponibilités, fonctions incompatibles, inaptitude médicale explicite et qualifications pont/machine manquantes. Les échéances de brevet, visite médicale ou autre document ne bloquent plus une affectation : une icône est affichée directement sur chaque case postérieure à l’échéance. Les blocages essentiels (activité, absence/indisponibilité et inaptitude médicale explicite) sont aussi contrôlés par trigger PostgreSQL. Les niveaux restent configurables dans `planning_rules`.

Une relève regroupe navire, instant, lieu, durée de passation, responsable, commentaires, statut et postes entrants/sortants. Sa sauvegarde complète passe par une RPC transactionnelle. La comparaison classe les postes inchangés, remplacés, vacants ou non conformes et expose les validations ou qualifications manquantes. Le workflow de dérogation est retiré ; les anciennes lignes restent en lecture historique pour préserver l’audit.
...
Les modèles restent dans `planning_templates`. Leur application crée soit un `planning_project`, soit une relève brouillon dans `planning_handovers`; aucune table d’événements concurrente n’est introduite. Les matrices `planning_manning_matrices` et `planning_manning_requirements` définissent les situations, fonctions, minima, cibles, brevets, qualifications, habilitations, formations et restrictions par navire. Le contrôle serveur quotidien détecte décision absente, fonctions non confirmées, capitaine absent ou multiple, minimum non atteint, cible non atteinte et document requis manquant. Les restrictions textuelles restent informatives tant qu’elles ne sont pas structurées.

Le module `/modules/fleet` expose le référentiel canonique `vessels` sous le nom « Navires », dans le menu « Opérations », ainsi que l’onglet « Décision d’effectif ». Sa visibilité est pilotée par rôle dans `role_module_permissions`, comme les autres menus administrables. Toutes les listes de navires continuent de lire cette table unique, initialement alimentée par la liste SharePoint QHSE `543b9f00-aed2-489a-808a-7b64cc835a83`. Une suppression fonctionnelle archive le navire afin de préserver ses références historiques. Chaque situation d’effectif versionne son genre de navigation, sa description d’activité — prédéfinie ou personnalisée — et ses prescriptions spéciales. Dans le Planning, une alerte administrateur ouvre la composition de la bordée : la fonction RH reste immuable et la fonction Planning est confirmée sur l’affectation continue complète. Seul le libellé RH exact `Capitaine` peut recevoir la fonction Planning `Capitaine`. Un brevet manquant reste visible et bloquant, sauf dérogation explicite, motivée et auditée dans `planning_staffing_derogations`. La RPC de publication refuse toute diffusion tant qu’un blocage subsiste ; un effectif compris entre minimum et cible reste un avertissement.
...
Le module `/modules/workingTime` fournit désormais une surface de production dédiée aux contrôles travail/repos. `WorkingTimePage.tsx` charge le même aperçu Planning, applique `getPlanningPermissions` et embarque `PlanningP13Panel.tsx` en présentation page limitée à l’onglet travail/repos. Les politiques, requêtes Supabase et calculs restent donc centralisés dans `planning_work_rest_policies`, `planningP13Queries.ts` et `planningP13.ts` ; aucune table, valeur par défaut ou règle parallèle n’est créée. Tous les rôles Planning autorisés peuvent consulter la surface, tandis que l’administration des politiques reste réservée au rôle administrateur dans l’interface et explicitement dans la RPC `save_planning_work_rest_policy`. Les politiques sont datées, globales ou propres à un navire, et couvrent les fenêtres glissantes de 24 heures et 7 jours, le repos consécutif, le fractionnement, la fenêtre de nuit, son maximum de travail et l’inclusion du temps de passation. Les valeurs issues d’un classeur ne peuvent être présentées que comme suggestions de configuration à vérifier par un administrateur ; elles ne sont ni activées automatiquement ni qualifiées de vérité légale.
...
| Aptitude et restrictions médicales | Partiel | Amélioré | Inaptitude explicite et restriction prises en compte ; échéance signalée par icône sans blocage | Maintenir la donnée RH |
| Certificats navires | Partiel | Partiel | Alertes à 90 jours ; pas encore de blocage selon opération | P1 |
| Centre de conflits | Absent | Opérationnel P1.2 | Neuf familles, responsable, priorité, statut, commentaire, source, historique et résolution | Maintenir |
...
| Temps de travail et repos | Partiel | Opérationnel P1.3 | Seuils administrables, 24 h/7 j, repos consécutif/fractionné, nuit et passation ; données détaillées historiques parfois non évaluables | Maintenir la qualité des saisies |
...
- `PlanningP03Panels.tsx` porte les vues Navire/Marin, l’éditeur de relève et la comparaison des bordées.
...
- `planning_derogations` : historique en lecture seule des anciennes exceptions ; aucune nouvelle écriture applicative n’est autorisée.
...
4. un instantané immuable des affectations, journées, périodes, projets, relèves, postes et anciennes exceptions historiques concernés est enregistré ;
...
Le périmètre peut couvrir toute la flotte ou un navire existant. Le trigger vérifie l’ancienne et la nouvelle période d’un événement : il empêche donc aussi de déplacer un événement hors d’une zone verrouillée ou vers une zone verrouillée. Relèves et postes sont soumis au même verrou ; les anciennes exceptions restent immuables. L’interface retire les contrôles d’édition, mais le trigger PostgreSQL reste l’autorité de sécurité.
...
| --- | --- | --- | --- | --- |
| `admin` | entreprise active | lecture/écriture | soumettre, valider, publier, réouvrir, archiver | navires, relèves, permissions, historique, export |
| `direction` | entreprise active | lecture/écriture | soumettre, valider, publier, réouvrir | historique, export |
...
- Les relations d’une affectation ou relève ne peuvent pas référencer une autre entreprise ; les anciennes relations d’exception conservent aussi cette protection d’intégrité.
...
- inaptitude médicale explicite toujours bloquante ;
...
- contrôles d’absence, signalement visuel d’un document échu et qualification manquante, sans dérogation active ;
...
- lecture des anciennes exceptions dans les instantanés et l’historique, sans nouvelle écriture ;
...
- traitement manuel, source, priorité, responsable, commentaire, résolution et historique, sans classement par dérogation ;
- permissions UI/RPC/RLS et garde serveur contre une affectation chevauchant une absence approuvée.
- seuils de travail/repos exclusivement administrés, passation incluse et fenêtres 24 h/7 j, sans exception ciblée ;
