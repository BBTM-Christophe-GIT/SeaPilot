...
La barre de commandes reste centrée sur les fonctions d’aide à la décision et les documents. Le groupe « Gestion des congés », « Actualiser », « Ouvrir un registre » et les anciennes actions globales de brouillon ont été retirés. Pour le profil Marin, « Enregistrer le brouillon » et « Valider » apparaissent ensemble sous le commentaire de la journée ; les autres profils ne reçoivent pas cette action de brouillon personnelle. Les fonctions secondaires s’ouvrent dans des fenêtres dédiées : « Import » pour l’assistant XLSM réservé aux administrateurs, « Exposition HSE / IMCA » pour les indicateurs et « Contrôles travail et repos » pour le moteur P1.3 et ses alertes.
...
| Brouillon personnel Marin enregistré sans soumission ni validation | `WorkingTimeEntryBoard.test.tsx`, `WorkingTimeWorkflowPanel.test.tsx` et `working_time_daily_approval_test.sql` |
