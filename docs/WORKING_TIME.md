...
Le workflow porte désormais sur une journée, pas sur tout le registre mensuel. Sous le commentaire, tout profil disposant du droit de saisie sur la journée bénéficie de « Enregistrer le brouillon » à côté de « Valider ». Le brouillon enregistre les périodes dans le registre sans créer de demande d’approbation ni figer de signature ; « Valider » signe et soumet la journée. Une signature active de l’auteur est obligatoire uniquement pour cette validation. La qualité de Capitaine vient exclusivement du libellé RH exact `Capitaine`, indépendamment du rôle applicatif : `Capitaine 200`, `Second capitaine` ou toute variante de casse ne confèrent pas cette qualité.

La saisie manuelle d’un mois reste ouverte jusqu’au 5 du mois suivant inclus, selon la date locale de Paris ; elle passe en lecture seule le 6. L’interface rappelle l’échéance pendant cette période de grâce puis indique explicitement la clôture. Le même verrou est appliqué dans les RPC de mutation. Une journée déjà soumise reste toutefois accessible à son approbateur après l’échéance afin de terminer la validation ou la justification d’une non-conformité.

Un repos continu atteignant le minimum configuré — 6 heures dans la politique BBTM — clôt le cycle précédent. Le compteur des 12 heures repart alors à zéro et le seuil de repos cumulé sur 24 heures ne rouvre pas ce cycle : la durée de repos réelle reste affichée, mais elle ne génère plus un écart `rest_24h` lorsque les 6 heures consécutives sont acquises. Les contrôles hebdomadaires, de nuit et de fractionnement restent indépendants. Cette règle est identique pour le calcul enregistré et pour les recommandations de saisie simple ou multiphase.
...
La barre de commandes reste centrée sur les fonctions d’aide à la décision et les documents. Le groupe « Gestion des congés », « Actualiser », « Ouvrir un registre » et les anciennes actions globales de brouillon ont été retirés. « Enregistrer le brouillon » et « Valider » apparaissent ensemble sous le commentaire de toute journée modifiable : Admin et Armement sur leur périmètre société, Capitaine sur son périmètre personnel ou de bordée publiée, et Marin sur sa propre feuille. La Direction conserve son accès en lecture seule. Les fonctions secondaires s’ouvrent dans des fenêtres dédiées : « Import » pour l’assistant XLSM réservé aux administrateurs, « Exposition HSE / IMCA » pour les indicateurs et « Contrôles travail et repos » pour le moteur P1.3 et ses alertes.
...
| Brouillon enregistré sans soumission ni validation pour chaque profil disposant d’un périmètre de saisie | `WorkingTimeEntryBoard.test.tsx`, `WorkingTimeWorkflowPanel.test.tsx`, `working_time_daily_approval_test.sql` et `working_time_workflow_permissions_test.sql` |
| Saisie du mois précédent ouverte jusqu’au 5 inclus, clôture le 6 et continuité des approbations | `workingTimeEntryWindow.test.ts`, `WorkingTimeWorkflowPanel.test.tsx` et `working_time_entry_window_test.sql` |
...
| Cycle conforme après 6 h de repos consécutif, même avec moins de 10 h cumulées sur 24 h | `working_time_work_cycle_capitaine_test.sql` |
