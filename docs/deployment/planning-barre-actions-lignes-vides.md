...
Appliquer `supabase/migrations/202607170001_planning_empty_board_rows.sql`, `supabase/migrations/202607170002_planning_board_row_eligibility.sql`, puis `supabase/migrations/20260824064058_allow_existing_planning_board_rows.sql`, avant de déployer le frontend. Ces migrations créent et actualisent :

- la table RLS `planning_board_rows` ;
- la fonction `add_planning_board_row`, limitée aux marins sans date de départ ou dont la date de départ est strictement postérieure à la date du jour, mais sans blocage lié à leur présence préalable dans la bordée ;
