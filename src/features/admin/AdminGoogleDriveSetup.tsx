...
export function AdminGoogleDriveSetup({ client, previewMode = false }: { client: SupabaseClient; previewMode?: boolean }) {
  ...
  return <section className="admin-panel admin-drive-setup" aria-labelledby="admin-drive-title">
    ...
    <ol className="admin-setup-steps">
      ...
      <li><div><h3>Installer le lanceur unique</h3><p>Extrayez l’archive puis exécutez <strong>Installer.cmd</strong> sur chaque PC. La version 2.2 permet de stocker les FDS sur Google Drive et de les intégrer aux exports chimiques. Une mise à jour conserve le dossier déjà configuré.</p><a className="admin-primary-button" href="/connectors/seapilot-drive-windows.zip?v=2.2.0" download><Download size={16} />Installer le lanceur Windows</a></div></li>
    ...
    <div className="admin-drive-usage"><h3>Classement automatique</h3><ul>{Object.entries(DRIVE_MODULES).map(([key, folder]) => <li key={key}><strong>{folder}</strong> : {key === 'disciplinary' ? 'un dossier par collaborateur, puis un sous-dossier par date.' : key === 'chemicals' ? 'un dossier par navire et par produit pour les FDS et pièces jointes.' : 'les fichiers de travail des procédures.'}</li>)}</ul><p>Les futurs modules utiliseront cette même racine. Enregistrez dans Word ou Excel puis laissez Google Drive terminer sa synchronisation.</p><p>Le dossier Sanctions Disciplinaires doit être partagé uniquement avec Administration et Direction. Partagez Produits Chimiques avec les utilisateurs concernés. Les droits de partage Google Drive restent à gérer dans Google Drive.</p></div>
  </section>;
}
