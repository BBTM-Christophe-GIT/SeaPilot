...
export function AdminGoogleDriveSetup({ client, previewMode = false }: { client: SupabaseClient; previewMode?: boolean }) {
  ...
  return <section className="admin-panel admin-drive-setup" aria-labelledby="admin-drive-title">
    ...
    <ol className="admin-setup-steps">
      ...
      <li><div><h3>Installer le lanceur unique</h3><p>Extrayez l’archive puis exécutez <strong>Installer.cmd</strong> sur chaque PC. La version 2.4 classe les pièces RH par collaborateur, crée les dossiers des modules, ouvre les nouvelles procédures et convertit les documents Office en PDF. Une mise à jour conserve le dossier déjà configuré.</p><a className="admin-primary-button" href="/connectors/seapilot-drive-windows.zip?v=2.4.0" download><Download size={16} />Installer le lanceur Windows</a></div></li>
