...
describe('disciplinary role and UI workflows', () => {
  ...
  it('selects a named active collaborator, explains choices and preserves editable letter text', async () => {
    ...
    await user.type(screen.getByLabelText('Faits observés'), 'Constat factuel de test.');
    ...
    expect((body as HTMLTextAreaElement).value).toContain('Constat factuel de test.');
    expect((body as HTMLTextAreaElement).value).toContain('ne constitue pas un fondement de dépistage des stupéfiants');
    ...
    expect(screen.getByLabelText('Corps du courrier modifiable')).toHaveValue('Texte modifié par la direction.');
  });
});
