...
export function LiftingItemForm({ initial, kind, busy, error, onClose, onSave }: {
  ...
  return <AppDialog title="Fiche matériel" eyebrow={kind === 'towing' ? 'Remorque maritime' : 'Apparaux de levage'} onClose={onClose} isBusy={busy}
    ...
    <div className="lifting-form-grid">
      <label>Identifiant du matériel<input readOnly value={draft.reference} placeholder="Attribué automatiquement à l’enregistrement" /><small className="lifting-muted">Identifiant conservé du registre source ou attribué automatiquement par navire et registre.</small></label>
      ...
      <label>Date de mise en service<input type="date" required readOnly={Boolean(initial.reference)} max={todayLocal()} value={draft.commissioned_on || todayLocal()} onChange={(e) => change('commissioned_on', e.target.value)} /></label>
