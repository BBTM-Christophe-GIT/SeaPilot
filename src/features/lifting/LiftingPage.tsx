...
import { ArrowLeft, CheckCircle2, Download, FileCheck2, Pencil, Plus, RotateCcw, Search, Trash2 } from 'lucide-react';
import { AppDialog } from '../../components/AppDialog';
import { supabase } from '../../lib/supabaseClient';
import type { RoleKey } from '../permissions/roles';
import type { AppShellOutletContext } from '../shell/AppShell';
import { LiftingIcon } from './LiftingIcons';
import { LiftingEntryForm, LiftingItemForm, LiftingStartForm } from './LiftingForms';
import { CONDITION_LABELS, INSPECTOR, KIND_LABELS, blankItem, canManageLifting, entryComplete, formatLiftingDate, type InspectionEntry, type ItemDraft, type LiftingInspection, type LiftingItem, type LiftingKind, type LiftingVessel } from './liftingModel';
import { downloadLiftingReport, fetchInspectionEntries, fetchLiftingRegister, fetchLiftingVessels, loadLiftingStamp, publishLiftingInspection, saveInspectionEntry, saveLiftingItem, setLiftingItemActive, startLiftingInspection } from './liftingQueries';
...
export function LiftingPage({ client, roles }: { client?: SupabaseClient; roles?: RoleKey[] }) {
  ...
  const [entryEditor, setEntryEditor] = useState<InspectionEntry | null>(null);
  ...
  const filtered = items.filter((i) => (showInactive || i.active) && `${i.reference} ${i.material_type} ${i.description} ${i.serial_number}`.toLocaleLowerCase('fr').includes(query.toLocaleLowerCase('fr')));
  ...
  async function reload() {
    ...
    try {
      ...
      if (id === requestId.current) { setItems(result.items); setInspections(result.inspections); }
    } catch (e) { if (id === requestId.current) setError(messageOf(e)); }
    finally { if (id === requestId.current) setLoading(false); }
  }
  useEffect(() => {
    setReport(null); setEntries([]); setItems([]); setInspections([]); setError(''); setNotice(''); setQuery(''); setYear('');
    void reload();
  ...
  return <section className="lifting-page">
    ...
    {section === 'crane' ? <div className="lifting-crane"><LiftingIcon kind="crane" /><div><h2>Examen à fond - Grue</h2><p>Cette section est réservée au contrôle de la grue. Les rapports existants restent disponibles dans les certificats du navire.</p><p>La première version du module couvre le registre des apparaux et les remorques.</p><Link to="/modules/certificates">Consulter les certificats flotte</Link></div></div> : <>
      <div className="lifting-toolbar"><label>Navire<select aria-label="Navire" value={vesselId} disabled={busy || !vessels.length} onChange={(e) => setVesselId(Number(e.target.value))}>{!vessels.length && <option value={0}>Aucun navire accessible</option>}{vessels.map((v) => <option value={v.id} key={v.id}>{v.name}</option>)}</select></label><div className="lifting-verifier"><span>Vérificateur</span><strong>{INSPECTOR}</strong></div></div>
      {error && !editor && !entryEditor && !startOpen && !publishOpen && !removeItem && <div className="lifting-error" role="alert">{error}<button onClick={() => { setError(''); void reload(); }}>Recharger</button></div>}
      {notice && <p className="lifting-notice" role="status"><CheckCircle2 size={18} />{notice}</p>}
      {!report ? <>
        <div className="lifting-summary"><div><strong>{activeItems.length}</strong><span>matériels en inventaire</span></div><div><strong>{inspections.filter((r) => r.status === 'draft').length}</strong><span>contrôles en cours</span></div><div><strong>{inspections.filter((r) => r.status === 'published').length}</strong><span>rapports finalisés</span></div><button className="primary-button" disabled={busy || loading || !activeItems.length} onClick={() => { setError(''); setStartOpen(true); }}><Plus size={18} /> Nouveau contrôle annuel</button></div>
        <div className="lifting-content"><div className="lifting-content-heading"><div className="lifting-tabs" role="group" aria-label="Vue du registre"><button aria-pressed={view === 'inventory'} onClick={() => setView('inventory')}>Inventaire</button><button aria-pressed={view === 'reports'} onClick={() => setView('reports')}>Contrôles et rapports</button></div>{manager && view === 'inventory' && <button className="secondary-button" disabled={busy || !vesselId} onClick={() => { setError(''); setEditor({ draft: blankItem(section) }); }}><Plus size={17} /> Ajouter un matériel</button>}</div>
          {loading ? <p className="lifting-empty" role="status">Chargement du registre…</p> : view === 'inventory' ? <>
            <div className="lifting-filter"><label className="lifting-search"><Search size={17} /><input aria-label="Rechercher un matériel" placeholder="Rechercher un identifiant, un matériel…" value={query} onChange={(e) => setQuery(e.target.value)} /></label><label className="lifting-toggle"><input type="checkbox" checked={showInactive} onChange={(e) => setShowInactive(e.target.checked)} /> Inclure les matériels supprimés</label></div>
            {!filtered.length ? <p className="lifting-empty">{query ? 'Aucun matériel ne correspond à la recherche.' : 'Aucun matériel dans cet inventaire.'}</p> : <div className="lifting-item-list">{filtered.map((item) => <article key={item.id} className={`lifting-item ${item.active ? '' : 'is-inactive'}`}><div className="lifting-id">{item.reference}</div><div className="lifting-item-main"><small>{item.material_type}{!item.active ? ' · Supprimé de l’inventaire actif' : ''}</small><h3>{item.description}</h3><p>{item.location || 'Emplacement non renseigné'}{item.serial_number && ` · N° ${item.serial_number}`}</p></div><div className="lifting-cmu"><span>CMU</span><strong>{item.swl_tonnes === null ? '—' : `${item.swl_tonnes} t`}</strong></div>{manager && <div className="lifting-row-actions"><button className="lifting-icon-button" title="Modifier" aria-label={`Modifier ${item.reference}`} disabled={busy} onClick={() => { setError(''); setEditor({ id: item.id, draft: { ...item } }); }}><Pencil size={17} /></button>{item.active ? <button className="lifting-icon-button" aria-label={`Supprimer ${item.reference}`} disabled={busy} onClick={() => { setError(''); setRemoveItem(item); }}><Trash2 size={17} /></button> : <button className="lifting-icon-button" aria-label={`Restaurer ${item.reference}`} disabled={busy} onClick={() => void act(async () => { await setLiftingItemActive(db, item.id, true); await reload(); })}><RotateCcw size={17} /></button>}</div>}</article>)}</div>}
          </> : <><div className="lifting-filter"><label>Année<select value={year} onChange={(e) => setYear(e.target.value)}><option value="">Toutes les années</option>{[...new Set(inspections.map((r) => r.inspection_year))].map((y) => <option key={y}>{y}</option>)}</select></label></div>{!displayedReports.length && <p className="lifting-empty">Aucun rapport pour cette sélection. Démarrez un contrôle annuel pour préparer le premier rapport.</p>}{displayedReports.map((r) => <article className="lifting-report-row" key={r.id}><FileCheck2 /><div><h3>{KIND_LABELS[r.kind]} · {r.inspection_year}</h3><p>Émission : {formatLiftingDate(r.issued_on)} · Échéance : {formatLiftingDate(r.expires_on)}</p></div><span className={`lifting-status ${r.status}`}>{r.status === 'draft' ? 'Brouillon' : 'Finalisé'}</span><button className="secondary-button" disabled={busy} onClick={() => void openReport(r)}>{r.status === 'draft' ? 'Reprendre' : 'Consulter'}</button><button className="lifting-icon-button" disabled={busy} aria-label={`Télécharger le rapport ${r.inspection_year}`} onClick={() => void exportReport(r)}><Download size={18} /></button></article>)}</>}
        </div>
      </> : <div className="lifting-content">
        <div className="lifting-report-heading"><button className="secondary-button" disabled={busy} onClick={() => { setReport(null); setView('reports'); void reload(); }}><ArrowLeft size={16} /> Rapports</button><div><h2>Contrôle annuel {report.inspection_year}</h2><p>{vessel?.name} · {formatLiftingDate(report.issued_on)} → {formatLiftingDate(report.expires_on)}</p></div><span className={`lifting-status ${report.status}`}>{report.status === 'draft' ? 'Brouillon' : 'Finalisé'}</span></div>
        <div className="lifting-progress">{report.source_label === 'PDF historique' && <p className="lifting-muted">Rapport historique importé. Les cases vides du document source restent non renseignées. Consultez le PDF original pour le contrôle signé.</p>}<div><strong>{report.source_label === 'PDF historique' ? `${entries.length} matériels dans le rapport source` : `${complete} / ${entries.length} matériels contrôlés`}</strong><span>{entries.filter((e) => ['repair','withdrawn','not_present'].includes(e.condition)).length} avec réserve, retrait ou non présentés</span></div><progress value={report.source_label === 'PDF historique' ? entries.length : complete} max={entries.length || 1} aria-label="Progression du contrôle" /></div>
        <div className="lifting-item-list">{entries.map((entry) => <article key={entry.id} className="lifting-item"><div className="lifting-id">{entry.item_snapshot.reference}</div><div className="lifting-item-main"><small>{entry.item_snapshot.material_type}</small><h3>{entry.item_snapshot.description}</h3>{entry.observations && <p>{entry.observations}</p>}</div><span className={`lifting-status ${entry.condition}`}>{CONDITION_LABELS[entry.condition]}{report.status === 'draft' && entry.condition !== 'pending' && !entryComplete(entry) ? ' · À compléter' : ''}</span>{report.status === 'draft' && <button className="secondary-button" disabled={busy} onClick={() => { setError(''); setEntryEditor(entry); }}>{entryComplete(entry) ? 'Modifier' : 'Contrôler'}</button>}</article>)}</div>
        <footer className="lifting-report-footer"><button className="secondary-button" disabled={busy} onClick={() => void exportReport(report)}><Download size={17} /> {report.status === 'draft' ? 'PDF brouillon' : 'Télécharger le PDF'}</button>{report.status === 'draft' && manager && <button className="primary-button" disabled={busy || !entries.length || complete !== entries.length} onClick={() => { setError(''); setPublishOpen(true); }}><FileCheck2 size={17} /> Finaliser et classer le rapport</button>}{report.status === 'published' && <Link className="primary-button" to="/modules/certificates">Voir dans Certificats flotte</Link>}</footer>
    ...
    {editor && section !== 'crane' && <LiftingItemForm initial={editor.draft} kind={section} busy={busy} error={error} onClose={() => { setEditor(null); setError(''); }} onSave={(draft) => void act(async () => { await saveLiftingItem(db, vesselId, section, draft, editor.id); setEditor(null); await reload(); setNotice('Matériel enregistré.'); })} />}
    {startOpen && section !== 'crane' && <LiftingStartForm busy={busy} error={error} onClose={() => { setStartOpen(false); setError(''); }} onSave={(issued, expires) => void act(async () => { const id = await startLiftingInspection(db, vesselId, section, issued, expires); const loaded = await fetchLiftingRegister(db, vesselId, section); const selected = loaded.inspections.find((r) => r.id === id); if (!selected) throw new Error('Contrôle créé. Rechargez le registre pour le retrouver.'); const rows = await fetchInspectionEntries(db, id); setInspections(loaded.inspections); setReport(selected); setEntries(rows); setStartOpen(false); })} />}
    {entryEditor && report && <LiftingEntryForm entry={entryEditor} busy={busy} error={error} onClose={() => { setEntryEditor(null); setError(''); }} onSave={(entry) => void act(async () => { const revision = await saveInspectionEntry(db, report, entry); setReport({ ...report, revision }); setEntries((previous) => previous.map((e) => e.id === entry.id ? entry : e)); setEntryEditor(null); setNotice('Contrôle enregistré. Vous pouvez le reprendre depuis un autre appareil.'); })} />}
