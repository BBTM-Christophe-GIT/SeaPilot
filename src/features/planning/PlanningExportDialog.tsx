...
export function PlanningExportDialog({
  ...
  const canGenerate = !saving && startsOn <= endsOn && selectedPersonIds.length > 0 && selectedVesselIds.length > 0
  ...
  async function runExport(event: FormEvent) {
    ...
    try {
      ...
      const result = await generatePlanningExport(exportKind, exportFormat, {
        overview,
        data,
        checks,
        startsOn,
    ...
    <div className="planning-dialog-backdrop" role="presentation">
      <section aria-label="Exports métier" aria-modal="true" className="planning-dialog planning-export-dialog" role="dialog">
        ...
        <form onSubmit={runExport}>
          ...
          {!selectedPersonIds.length || !selectedVesselIds.length ? <p className="planning-export-feedback is-error" role="alert">Sélectionnez au moins un marin et un navire.</p> : null}
