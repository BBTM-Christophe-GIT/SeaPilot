          ...
          <div className="planning-silae-people">
            {employees.map((employee) => <div className="planning-silae-person" key={employee.person.id}>
              ...
              <details><summary>{employee.issues.length ? `${employee.issues.length} point${employee.issues.length > 1 ? 's' : ''} à résoudre` : 'Vérifier les périodes'}</summary>
                ...
                <div className="planning-silae-periods"><table><caption>Périodes de {silaePersonName(employee.person)}</caption><thead><tr><th>Du</th><th>Au</th><th>État</th><th>Navire</th><th>JrsMer</th><th>JrsEmbarque</th></tr></thead><tbody>{employee.periods.map((period) => <tr key={period.startsOn}><td>{formatPlanningDate(period.startsOn)}</td><td>{formatPlanningDate(period.endsOn)}</td><td>{period.state === 'sea' ? 'En mer' : 'En repos'}</td><td>{period.registrationNumber || 'À renseigner'}</td><td>{period.seaDays}</td><td>{period.embarkedDays}</td></tr>)}</tbody></table></div>
              </details>
            </div>)}
