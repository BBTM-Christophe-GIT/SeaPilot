...
function inconsistencySuggestions(
  ...
  const suggestions = conflicts.filter((item) => !excluded.has(item.type)).slice(0, 30).map<PlanningAssistantSuggestion>((item) => ({
    ...
    suggestedSteps: ['Ouvrir les éléments concernés.', 'Choisir une correction manuelle.', 'Traiter ou déroger via le workflow P1 existant.'],
    candidates: [],
    vesselId: item.vesselId,
    personId: item.personId,
    humanValidationRequired: true,
  ...
  const workRestSuggestions = workRestChecks.filter((check) => check.status === 'non_compliant').slice(0, 10)
    .map<PlanningAssistantSuggestion>((check) => ({
      ...
      rulesApplied: ['Politique travail/repos P1.3', 'Dérogations actives prises en compte'],
      ...
      suggestedSteps: ['Vérifier les heures sources.', 'Réorganiser manuellement le service si nécessaire.', 'Utiliser le workflow de dérogation uniquement après décision autorisée.'],
      candidates: [],
  ...
  return conflicts.filter((item) => reorganizable.has(item.type) && item.vesselId).slice(0, 12).map((item) => {
    ...
    return {
      ...
      suggestedSteps: compatible
        ...
        : ['Étudier un décalage de dates.', 'Revoir le besoin de fonction.', 'Escalader le conflit sans appliquer automatiquement de dérogation.'],
      candidates,
      vesselId: item.vesselId,
