...
describe('Planning preview data', () => {
  it('provides a vessel, watch and sailor hierarchy without exposing an empty vessel', () => {
    ...
    const pendingRows = buildPlanningCrewRows(overview, buildPlanningTimeline(anchorDate, 'month'), {
      vesselName: '', personName: '',
    }, undefined, { pendingBoardRowIds: new Set([1201]) });
    expect(pendingRows.some((row) => row.type === 'person' && row.label === 'Alain ANCIEN' && !row.events.length)).toBe(true);
