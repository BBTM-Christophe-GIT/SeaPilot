...
export async function fetchPlanningSilaeData(client: SupabaseClient, month: string): Promise<SilaeData> {
  ...
  const [peopleRows, vesselRows, periods, assignments, days, absences] = await Promise.all([
    ...
    readRows(client, 'planning_assignments', 'id,crew_person_id,vessel_id,starts_on,ends_on,status_label,confirmation_status,assignment_role,updated_at', 'starts_on', range.end),
  ...
  const sources: SilaeSource[] = periods.map((row) => ({
    personId: sourcePerson(row), vesselId: id(row.vessel_id), startsOn: text(row.starts_on), endsOn: text(row.ends_on), status: text(row.sailor_status), functionLabel: text(row.function_label), priority: 1, sourceId: Number(row.id),
  }));
  sources.push(...validAssignments.map((row) => ({
    personId: id(row.crew_person_id), vesselId: id(row.vessel_id), startsOn: text(row.starts_on), endsOn: text(row.ends_on), status: text(row.status_label), functionLabel: text(row.assignment_role), priority: 2, updatedAt: text(row.updated_at), sourceId: Number(row.id),
  })));
  days.forEach((row) => {
    ...
    sources.push({ personId: sourcePerson(row), vesselId: id(row.vessel_id), startsOn: date, endsOn: date, status: text(row.sailor_status) || text(row.day_status), functionLabel: text(row.function_label), priority: 3, sourceId: Number(row.id), parentAssignmentId: row.source_label === PLANNING_ASSIGNMENT_NOTE_SOURCE ? Number(assignment!.id) : undefined });
  });
