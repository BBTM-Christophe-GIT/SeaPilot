...
describe('AppShell', () => {
  it('renders the private application navigation', async () => {
    ...
    expect(screen.queryByRole('link', { name: 'Éléments de facturation' })).not.toBeInTheDocument();
  ...
  it('allows a Marin profile to open the DPR module directly', async () => {
    const client = {
      auth: {
        getSession: vi.fn().mockResolvedValue({ data: { session: { user: { id: 'user-1' } } }, error: null }),
        onAuthStateChange: vi.fn().mockReturnValue({ data: { subscription: { unsubscribe: vi.fn() } } }),
    ...
    render(
      <AuthProvider client={client as never}>
        <MemoryRouter initialEntries={['/modules/dpr']}>
          <Routes>
            <Route element={<AppShell rolesOverride={['marin']} />}>
              <Route path="modules/dpr" element={<div>Formulaire DPR marin</div>} />
            </Route>
          </Routes>
        </MemoryRouter>
      </AuthProvider>,
    ...
    expect(await screen.findByText('Formulaire DPR marin')).toBeInTheDocument();
