...
async function syntheticWorkbook(options: { reportedHours?: number; macro?: boolean } = {}): Promise<Uint8Array> {
  const zip = new JSZip();
  ...
  const workCells = ['D4', 'E4', 'H4', 'I4'].map((address) => `<c r="${address}" s="1"/>`).join('');
