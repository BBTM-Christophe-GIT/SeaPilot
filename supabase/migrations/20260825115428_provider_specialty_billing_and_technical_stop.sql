-- Preserve the supplier specialties used on each chargeable expense and add
-- the technical-stop visit type to the Planning workflow.

alter table public.project_chargeable_expenses
  add column if not exists supplier_specialties text[] not null default '{}'::text[];

with specialty_snapshots as (
  select
    expense.id,
    coalesce(
      (
        select array_agg(distinct specialty.name order by specialty.name)
        from public.service_providers provider
        join public.service_provider_specialties specialty
          on specialty.provider_id = provider.id
         and specialty.company_id = provider.company_id
         and specialty.active
        where provider.company_id = expense.company_id
          and provider.merged_into_provider_id is null
          and lower(trim(provider.name)) = lower(trim(expense.supplier))
      ),
      case
        when nullif(trim(expense.nature), '') is not null then array[trim(expense.nature)]
        else '{}'::text[]
      end
    ) as specialties
  from public.project_chargeable_expenses expense
  where cardinality(expense.supplier_specialties) = 0
)
update public.project_chargeable_expenses expense
set supplier_specialties = snapshot.specialties
from specialty_snapshots snapshot
where expense.id = snapshot.id
  and cardinality(snapshot.specialties) > 0;

comment on column public.project_chargeable_expenses.supplier_specialties is
  'Snapshot of active supplier specialties displayed in billing tables and PDF exports.';

alter table public.vessel_visits
  drop constraint if exists vessel_visits_type_check;
alter table public.vessel_visits
  add constraint vessel_visits_type_check check (visit_type in (
    'water_analysis',
    'client_audit',
    'imca_audit',
    'internal_audit',
...
begin
  ...
  if target_company_id is null
    ...
    or p_visit_type not in (
      ...
      'crane_visit', 'fire_visit', 'qhse_visit', 'technical_stop'
    )
...
revoke all on function public.save_vessel_visit(bigint, bigint, text, bigint, text, timestamptz[])
  from public, anon;
grant execute on function public.save_vessel_visit(bigint, bigint, text, bigint, text, timestamptz[])
  to authenticated;
