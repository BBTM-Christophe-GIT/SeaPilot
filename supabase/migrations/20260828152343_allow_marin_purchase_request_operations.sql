...
begin
  ...
  where request.id = p_request_id
    and (
      ...
      or (
        p_action = any(array['take_charge', 'plan_delivery', 'mark_received'])
...
comment on function public.purchase_request_transition(bigint, text, text, date) is
  'Transitions métier sécurisées par entreprise et rôle ; le profil Marin est limité au traitement logistique.';
